...
import 'package:em_app/pages/employee/employee_service.dart'; 
import 'package:em_app/pages/employee/employee_lib.dart'; 
import 'package:em_app/pages/employee/employee_edit.dart'; 
 
class EmployeeDataTable extends StatefulWidget { 
  final String? viewType; 
  const EmployeeDataTable({Key? key, this.viewType}) : super(key: key); 
 
  @override 
  _EmployeeDataTableState createState() => _EmployeeDataTableState(); 
} 
 
class _EmployeeDataTableState extends State<EmployeeDataTable> { 
  String dispType = "DataTable"; 
  String emTitle =  "Employee Data Table"; 
  ...
  void initState() { 
    ...
    if (widget.viewType != null ) { 
      ...
      switch (widget.viewType) { 
        case "ListView": { 
        dispType = "ListView"; 
        emTitle = "Employee List View"; 
        } 
        break; 
        case "ListCard": { 
        dispType = "ListCard"; 
        emTitle = "Employee List Card"; 
        } 
        break; 
        default: 
      } 
    ...
    _loading = true; 
    print("Calling Service : viewAllEmployee ... "); 
    EmployeeService.EmployeeViewAll().then((getList) { 
...
Container getDataTableView() { 
          ...
          return Container(  
                ...
                DataColumn( 
                    label: const Text('Mobile'), onSort: doColumnSort), 
                      const DataColumn( 
                        label: Text( 
                          'Edit/Delete', 
...
// --------------- List Card View ------------------- 
  GridView getListCard() { 
    ...
    return GridView.count( 
        ...
        children: List.from(EmployeeCardList) 
    ); 
  } 
 
  Widget getColumnsForListCard1(Employee e) { 
...
// --------------- List View ------------------- 
  ...
  DataTable _buildListView() { 
    return DataTable( 
      columns: <DataColumn>[ 
        DataColumn(label: Text(employeeList[0].toListLabVal()[0][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))), 
        DataColumn(label: Text(employeeList[0].toListLabVal()[1][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))), 
      ], 
      rows: employeeList 
          .map((e) => DataRow( 
                cells: [ 
...
// --------------- DataTable Functions ------------------- 
  void doColumnSort(int columnIndex, bool ascending) { 
	if (columnIndex == 0) { 
	  ...
	  employeeList.sort((item1, item2) => 
		  compareColumn(ascending, item1.mobile, item2.mobile)); 
    } 
...
class EmployeeDataSource extends DataTableSource { 
  ...
  @required 
  late List<Employee> _data; 
 
  EmployeeDataSource(List<Employee> employeeList) { 
    _data = employeeList; 
  } 
  ...
  DataRow getRow(int index) { 
	...
	DataCell(Text(getEmStrOpt(_data[index].mobile))), 
      DataCell(Row( 	
            ...
            return IconButton( 	
                onPressed: () => showDialog<String>( 	
                      ...
                      builder: (BuildContext context) => AlertDialog( 	
                        ...
                        content: _EmployeeInfo(foundEmployee: _data[index]), 	
                        actions: <Widget>[ 	
                          TextButton( 	
...
// --------------- DataTable Delete Call Functions - View Record -------------------	
	...
	class _EmployeeInfo extends StatelessWidget { 	
	  ...
	  const _EmployeeInfo({Key? key, required this.foundEmployee}) 	
	      : super(key: key); 	
	 	
	  @override 	
	  Widget build(BuildContext context) { 	
			return Employee_dispForm(context, foundEmployee); 	
	  } 	
	} 	
