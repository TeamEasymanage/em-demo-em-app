...
class StudentDataTable extends StatefulWidget { 
  ...
  const StudentDataTable({Key? key, this.viewType}) : super(key: key); 
...
class _StudentDataTableState extends State<StudentDataTable> { 
  ...
  String emTitle =  "Student Data Table"; 
  ...
  void initState() { 
    ...
    if (widget.viewType != null ) { 
      ...
      switch (widget.viewType) { 
        case "ListView": { 
        dispType = "ListView"; 
        emTitle = "Student List View"; 
        } 
        break; 
        case "ListCard": { 
        dispType = "ListCard"; 
        emTitle = "Student List Card"; 
        } 
        break; 
        default: 
      } 
    ...
    _loading = true; 
    print("Calling Service : viewAllStudent ... "); 
    StudentService.StudentViewAll().then((getList) { 
...
Container getDataTableView() { 
          ...
          return Container(  
                   ...
                   //------------------------------------ Scrollbars 	
                      ...
                      source: StudentDataSource(studentList), 
                      columns: <DataColumn>[ 
                DataColumn( 
                    label: const Text('Name'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Marks'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Phone'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Roll'), onSort: doColumnSort), 
                      const DataColumn( 
                        label: Text( 
                          'Edit/Delete', 
...
// --------------- List Card View ------------------- 
  GridView getListCard() { 
    ...
    return GridView.count( 
        ...
        children: List.from(StudentCardList) 
    ); 
  } 
 
  Widget getColumnsForListCard1(Student e) { 
...
// --------------- List View ------------------- 
  ...
  DataTable _buildListView() { 
    return DataTable( 
      columns: <DataColumn>[ 
        DataColumn(label: Text(studentList[0].toListLabVal()[0][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))), 
        DataColumn(label: Text(studentList[0].toListLabVal()[1][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))), 
      ], 
      rows: studentList 
          .map((e) => DataRow( 
                cells: [ 
...
// --------------- DataTable Functions ------------------- 
  void doColumnSort(int columnIndex, bool ascending) { 
	if (columnIndex == 0) { 
	  studentList.sort((item1, item2) => 
		  compareColumn(ascending, item1.name, item2.name)); 
	} else if (columnIndex == 1) { 
	  studentList.sort((item1, item2) => 
		  compareColumn(ascending, item1.marks, item2.marks)); 
	} else if (columnIndex == 2) { 
	  studentList.sort((item1, item2) => 
		  compareColumn(ascending, item1.phone, item2.phone)); 
	} else if (columnIndex == 3) { 
	  studentList.sort((item1, item2) => 
		  compareColumn(ascending, item1.roll, item2.roll)); 
    } 
...
class StudentDataSource extends DataTableSource { 
  ...
  StudentDataSource(List<Student> studentList) { 
    _data = studentList; 
  } 
  ...
  DataRow getRow(int index) { 
	...
	DataCell(Text(getEmStrOpt(getEmFmtNumOpt(_data[index].marks)))), 
	DataCell(Text(getEmStrOpt(getEmFmtNumOpt(_data[index].phone)))), 
	DataCell(Text(getEmStrOpt(getEmFmtNumOpt(_data[index].roll)))), 
      DataCell(Row( 	
            ...
            return IconButton( 	
                onPressed: () => showDialog<String>( 	
                      ...
                      builder: (BuildContext context) => AlertDialog( 	
                        ...
                        content: _StudentInfo(foundStudent: _data[index]), 	
                        actions: <Widget>[ 	
                          TextButton( 	
...
// --------------- DataTable Delete Call Functions - View Record -------------------	
	...
	class _StudentInfo extends StatelessWidget { 	
	  ...
	  const _StudentInfo({Key? key, required this.foundStudent}) 	
	      : super(key: key); 	
